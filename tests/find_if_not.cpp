...
int test1()
{
  try
  {
    ...
    DYNAMIC_VERIFY( stdex::find_if_not(con.begin(), con.end(), predicate).ptr == array );
  }
  catch(int line)
  {
    return line;
...
int test2()
{
  try
  {
    ...
    DYNAMIC_VERIFY( stdex::find_if_not(con.begin(), con.end(), predicate).ptr == array + 1 );
  }
  catch(int line)
  {
    return line;
