...
namespace thread_cpp_detail
{
...
#if defined(_STDEX_CHRONO_CLOCK_MONOTONIC) && defined(TIMER_ABSTIME)
	template<>
	struct nanosleep_impl1<true>
	{
		...
		static int call(const timespec *req, timespec *rem)
		{
			...
			int err = ::clock_gettime(_STDEX_CHRONO_CLOCK_MONOTONIC, &tp);
			if(err != 0)
				return err;

			timespec_add(tp, *req);
			
			err = ::clock_nanosleep(_STDEX_CHRONO_CLOCK_MONOTONIC, TIMER_ABSTIME, &tp, rem);
